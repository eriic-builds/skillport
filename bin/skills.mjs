...
const commitTrailers = [
  ...
  "Copilot-Session: 01328afd-c01b-4342-b402-58969955671d",
];
...
function removeLinks(skillName) {
  ...
  const codexLink = join(codexRoot, skillName);
  if (existsSync(codexLink) && lstatSync(codexLink).isSymbolicLink()) {
    try {
      const target = realpathSync(codexLink);
  ...
  const agentsLink = join(agentsRoot, skillName);
  if (existsSync(agentsLink) && lstatSync(agentsLink).isSymbolicLink()) {
    try {
      const target = realpathSync(agentsLink);
      if (target.startsWith(repoRoot)) {
        rmSync(agentsLink);
      }
    } catch {
...
function unshelveSkills(names) {
  ...
  for (const name of names) {
    ...
    const skillsTarget = join(skillsDir, name);
    renameSync(skill.directory, skillsTarget);
  }
...
async function importSkill(url, options = {}) {
  ...
  try {
    ...
    for (const selected of chosen) {
      ...
      stage = join(targetRoot, `.import-${skill.folder}-${process.pid}`);
      ...
      stage = null;
      installed.push({ folder: skill.folder, target });
      const location = isShelfImport ? "shelf" : "skills";
      console.log(`Imported ${skill.folder} to ${location}/ from ${url} at ${sha}.`);
    }
    ...
    // Leave the library exactly as it was rather than half-importing a set.
...
Commands:
  ...
  doctor               Validate skills, client links, and Git state
  sync [message]       Commit, rebase-pull, and push library changes safely
  import <GitHub URL> [skill]
